...
[[ $(yq eval-all -rN 'select(.kind == "Prometheus") | .spec.retentionSize' "$monitoring_chart") == 30GB ]]
...
[[ $(yq eval-all -rN 'select(.kind == "Prometheus") | .spec.storage.volumeClaimTemplate.spec.resources.requests.storage' "$monitoring_chart") == 40Gi ]]
...
[[ $(yq eval-all -rN 'select(.kind == "StatefulSet" and .metadata.name == "loki") | .spec.template.spec.securityContext.seccompProfile.type' "$loki_chart") == RuntimeDefault ]]
...
[[ $(yq eval-all -rN 'select(.kind == "StatefulSet" and .metadata.name == "loki") | .spec.volumeClaimTemplates[0].spec.resources.requests.storage' "$loki_chart") == 40Gi ]]
