...
current-plan summary before its decision transcript.
`DATABASE_ACCESS_POLICY.md` proposes retained database lifecycle, declarative
account management, campus-network access, and privileged administration.
`VAULT.md` now contains only activation status, migration follow-up, and links
to the durable component contracts under `argocd/platform/vault/`,
`argocd/charts/application/`, and `workers/kms/`.
