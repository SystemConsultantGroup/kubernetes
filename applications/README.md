...
호스트 이름을 사용하며 `115.145.150.0/24`의 인터넷 클라이언트만 허용합니다.
전체 이름 및 라우팅 규칙은 차트 README를 참조하세요.

이 워크로드의 관리형 Gateway 응답에 요소를 삽입하려면 비어 있지 않은
`http.inject` 문자열을 추가하세요.
...
이 필드가 있으면 플랫폼은 해당 관리형 HTTPRoute 규칙에 Envoy Gateway Wasm
응답 필터를 연결합니다. 워크로드 Service에 대한 직접적인 클러스터 내부 호출은
변경되지 않습니다. 값은 공개 구성이므로 시크릿을 포함하면 안 됩니다. 애플리케이션은
`127.0.0.1` 또는 wildcard 주소에서 수신해야 합니다. 고정된 응답 선택 및 CSP 동작은
차트 README를 참조하세요.
