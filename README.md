...
change under [`applications/`](applications/README.md) or
[`argocd/`](argocd/README.md) can change
the live cluster.

## Application owners

Work in [`applications/`](applications/README.md) and submit a pull request.
...
[`applications/` guide](applications/README.md) for the file formats, examples, preview
...
> [`patches/`](patches/README.md) before an operation. `k reset` wipes the selected
...
- [`applications/`](applications/README.md) contains application metadata,
  digest-pinned instance lock files, and custom Kustomizations.
- [`argocd/`](argocd/README.md) contains the GitOps root, ApplicationSets,
  platform components, projects, and the shared application chart.
- [`patches/`](patches/README.md) contains shared and node-specific Talos patches.
- [`scripts/`](scripts/README.md) provides the `k` operator command and its help.
- [`secrets/`](secrets/README.md) contains the public recipient registry and
...
- [`workers/`](workers/README.md) contains Cloudflare Workers deployed outside Argo CD.
